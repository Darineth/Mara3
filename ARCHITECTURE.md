...
- **Updates (portable, in-place):** the client stays a portable single exe and updates itself by swapping that exe, never by running an installer. Two hosts, deliberately distinct: `scripts/package.mjs` bakes `MARA_UPDATE_URL = <MARA_MANIFEST_BASE_URL>/latest-<os>.json` (default the CORS-enabled raw-content copy of `updates/`) for *reading* the manifest, while the manifest's own `url` points at `<MARA_UPDATE_BASE_URL>/<archive>` (default the GitHub Releases `latest` endpoint) for *downloading* it. That split is load-bearing: the releases endpoint redirects to a host that answers without CORS headers, so pointing the manifest there made every in-page check fail with "Update check failed" — which is what shipped through 3.0.33. `lib.rs` injects `window.__MARA_UPDATE__ = { current, manifestUrl, canInstall }` on **every** page it loads. Three surfaces read it: the bootstrap picker (`bootstrap/index.html`), which asks Rust's `check_update` for the manifest (no CORS to lose) and offers **Update now** plus a Download link; the hosted web UI (`apps/web` `UpdateBanner.svelte` via `lib/update.ts`), which fetches it from the page and so still needs the CORS header; and the Android build, which checks but shows only the link (`canInstall: false` — a sideloaded APK is replaced through Android's installer). The install itself (`src-tauri/src/update.rs` → `crates/mara-swap`) downloads, verifies the manifest's `sha256`, unpacks to a staging dir inside the install, renames the running exe to `<name>.old`, moves the new one in, and relaunches; the next launch deletes the backup. `settings.json`, logs and `webview2-runtime/` are never touched. It runs only from the local picker (granted in `capabilities/default.json`, withheld from `grant_remote_ipc`) and Rust re-reads the baked manifest rather than trusting the caller, so a connected server can neither trigger an update nor choose its payload. The manifest is emitted by `scripts/zip-dist.mjs` (`{ version, url, notes, pub_date, sha256 }`) for windows-x64, windows7-x64, linux-x64 and android-arm64; build with `MARA_UPDATE_URL=` (empty) to disable the check. The **Win7 legacy client** (`apps/client-legacy`, Tauri 1) has the same flow against its own `latest-windows7-x64.json`, sharing `crates/mara-swap` so the file work can't drift, with its picker-only guard as a runtime origin check (Tauri 1 has no per-command ACL). Tauri's own updater plugin stays unused: it installs app *bundles*, which is the model this client exists to avoid.
